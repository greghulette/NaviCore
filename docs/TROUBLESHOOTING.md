...
| Board reboots every time the config-tool page is refreshed | Read the boot line `Reset reason: N - …` first — it splits the causes. **Power-on / EN reset** = the USB Serial/JTAG peripheral pulsed EN when the browser toggled the CDC control lines (native USB on NaviCore v2 has no bridge chip; these lines are how esptool resets an S3). **Task watchdog** = `loop()` stalled during the USB teardown instead. A `pagehide` handler now deasserts DTR/RTS and closes the port, which helps only if the reset fires on the CLOSE edge | `_releaseSerialOnUnload()`, `printBootTelemetry()` |

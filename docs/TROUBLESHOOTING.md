...
| Client on the droid's AP still shows Default Gateway `192.168.4.1` after a firmware update | **The client has not re-leased — not a firmware fault.** A renewal is a REQUEST/ACK, and Windows keeps the previous gateway when the ACK simply omits option 3, so a plain `ipconfig /renew` proves nothing. Run `ipconfig /release "<adapter>"` **then** `ipconfig /renew "<adapter>"`, or reconnect: expect a `192.168.4.x` address (not 169.254.x), an empty gateway, and no `0.0.0.0/0` route via `192.168.4.1` in `Get-NetRoute -DestinationPrefix 0.0.0.0/0`. The boot log settles it: `[WIFI] DHCP offers no default gateway` means the firmware did its part; `could not read…` / `could not clear the DHCP gateway option` means it did not | `setup()`, [PROTOCOLS.md §1](PROTOCOLS.md#1-transport-overview) |
| Clients associate to the AP but get no address — 169.254.x | The AP's DHCP server is not running. `[WIFI] *** DHCP server FAILED to restart` at boot is exactly this: the no-gateway step stops and restarts it, and nothing else will start a server left stopped. With no such line, look for a `WiFi.softAPConfig()` call — on the WCB it produced this silently, before or after `softAP()`, which is why NaviCore never calls it | `setup()`, [PROTOCOLS.md §1](PROTOCOLS.md#1-transport-overview) |

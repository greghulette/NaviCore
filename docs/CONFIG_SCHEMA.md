...
| `handleOtaEndPacket()` sends `otaWrittenOffset()` **after** `otaEnd()` clears `ota.active` | `otaSendAwaitAck(..., a => a.offset === 0)` at the END call | END ACK carries offset **0**; a DATA cursor ACK never does. It is the only field separating them — same session, same src, same status 0 |
